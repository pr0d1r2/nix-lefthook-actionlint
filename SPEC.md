...
| B12 | 2026-09-14 | Guardrails ran the tracked Bats suite's `lefthook-tdd-order-bats`, but the consumer flake omitted the `bats` fragment from its devShell materialization | Include the `bats` fragment in all flake materialization lists |
| B13 | 2026-09-23 | Tracked Bats tests loaded undeclared `bats-support` and `bats-assert` libraries, so guardrails failed every test during `setup()` | Keep the tests self-contained with local success/failure assertions and provide Bats in the devShell |
