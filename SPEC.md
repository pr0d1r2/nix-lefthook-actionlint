...
Lefthook-compatible actionlint wrapper as Nix flake. Filter .yml/.yaml from args, run actionlint, exit 0 when no match.
...
- C3: Timeout configurable via `LEFTHOOK_ACTIONLINT_TIMEOUT` env var, default 30s
...
- I.remote: `lefthook-remote.yml` — drop-in lefthook remote config (pre-commit + pre-push)
- I.env: `LEFTHOOK_ACTIONLINT_TIMEOUT` — seconds, default 30
...
| id | st | desc | cites |
|----|----|------|-------|
| T1 | x | Shell wrapper: filter yaml, exec actionlint | V1-V8,I.cli |
| T2 | x | Nix flake: writeShellApplication package | I.flake-pkg,C1 |
| T3 | x | Nix flake: devShell via nix-dev-shell-agentic | I.flake-dev |
| T4 | x | lefthook-remote.yml for consumers | I.remote,C3 |
| T5 | x | lefthook.yml for self (dev hooks) | I.cli |
| T6 | x | Bats unit tests (7 cases) | V1-V8 |
| T7 | x | CI workflow: 3 platforms via nix-lefthook-ci-action | C2 |
| T8 | x | README with usage docs | I.remote,I.flake-pkg |
| T9 | . | Create GitHub repo and push | |
| T10 | . | Verify CI passes on all 3 platforms | C2,T7 |
