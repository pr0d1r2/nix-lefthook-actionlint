...
| B4 | 2026-08-04 | Flake manifest check rejected implementation logic in the top-level `outputs` body | Move output construction to `flake/default.nix` and leave `flake.nix` as a thin manifest |
| B5 | 2026-08-04 | Classic nixfmt check rejected the newly extracted `flake/default.nix` because it was left with inconsistent indentation | Format `flake/default.nix` with the repository's classic nixfmt style |
